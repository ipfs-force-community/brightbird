...
var log = logging.Logger("damocles-worker-gpu-deployer")
...
type DamoclesWorkerGpuReturn struct { //nolint
	VConfig
...
//go:embed  damocles-worker-gpu
var f embed.FS

func DeployFromConfig(ctx context.Context, k8sEnv *env.K8sEnvDeployer, cfg Config) (*DamoclesWorkerGpuReturn, error) {
	...
	// create configMap
	configMapCfg, err := f.Open("damocles-worker-gpu/damocles-worker-gpu-configmap.yaml")
	if err != nil {
		return nil, err
	}
	...
	// create deployment
	deployCfg, err := f.Open("damocles-worker-gpu/damocles-worker-gpu-statefulset.yaml")
	if err != nil {
		return nil, err
	}
	...
	// create service
	svcCfg, err := f.Open("damocles-worker-gpu/damocles-worker-gpu-headless.yaml")
	if err != nil {
		return nil, err
	}
	...
	return &DamoclesWorkerGpuReturn{
		VConfig: cfg.VConfig,
...
func GetPods(ctx context.Context, k8sEnv *env.K8sEnvDeployer, instanceName string) ([]corev1.Pod, error) {
	return k8sEnv.GetPodsByLabel(ctx, fmt.Sprintf("damocles-worker-gpu-%s-pod", env.UniqueId(k8sEnv.TestID(), k8sEnv.Retry(), instanceName)))
}
