...
var (
	...
	listenAddress      = flag.String("listen", "", "Specify an address to accept HTTP requests, e.g.\"")
...
func main() {
	...
	flag.Usage = usage
	flag.Var(&mirrorPaths, "mirror-path", "Directory to which git repositories should be mirrored")
	flag.Parse()

	// Show some basic config info
	log.Println("Git repositories will be mirrored to: ", mirrorPaths)

	// Start the listening web server
	handler, err := NewHandler(mirrorPaths)
	if err != nil {
